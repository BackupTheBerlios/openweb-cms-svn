...
<xsl:template match="subject[@role]">
...
<xsl:template name="jolisubjectterm">
  <xsl:param name="term"/>
  <xsl:variable name="classement" select="$doc.criteres/criteres/critere/classements/entry[normalize-space(name/text()) = normalize-space($term)]"/>
  <a>
